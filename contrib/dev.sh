...
BCLI="$COMPOSE exec -T --user bitcoind bitcoind  bitcoin-cli -regtest "
CLN_CLI="$COMPOSE exec -T cln lightning-cli --network=regtest "
LND_CLI="$COMPOSE exec -T lnd lncli --network=regtest "
...
gen_blocks() {
  ...
  if [ -z $($BCLI listwallets | grep 'miner') ]; then
...
gen_addr() {
  ...
  if [ -z $($BCLI listwallets | grep $wallet) ]; then
...
copy_cert() {
  local root="./data"
  _cp "$COMPOSE exec -T cln cat" "/root/.lightning" "$root/cln" "/regtest/" "ca.pem"
  _cp "$COMPOSE exec -T cln cat" "/root/.lightning" "$root/cln" "/regtest/" "client.pem"
  _cp "$COMPOSE exec -T cln cat" "/root/.lightning" "$root/cln" "/regtest/" "client-key.pem"
  _cp "$COMPOSE exec -T lnd cat" "/root/.lnd" "$root/lnd" "/" "tls.cert"
  _cp "$COMPOSE exec -T lnd cat" "/root/.lnd" "$root/lnd" "/data/chain/bitcoin/regtest/" "admin.macaroon"
}
...
*)
  echo "Usage: dev.sh {prepare|gen|connect|fund|open|pay|test|copy_cert|lnd|cln}"
  exit 2
  ;;
esac
exit 0
